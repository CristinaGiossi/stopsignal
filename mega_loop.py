...
def mega_loop(self):
    ...
    #for action_idx in range(len(actionchannels)):
        ...
        #agent.FreqExt_AMPA_stop_basestim[popid] = agent.FreqExt_AMPA[popid]
        #print("agent.FreqExt_AMPA_stop_basestim[popid]",agent.FreqExt_AMPA_stop_basestim[popid])
    ...
    while self.trial_num < self.n_trials:
        ...
        #print('agent.ramping_stopstim_current', agent.ramping_stopstim_current)
            ...
            if agent.phase == 0 or agent.phase == 1:
                ...
                if agent.phasetimer>trial_wise_stop_onset+self.stop_duration_df.iloc[self.trial_num][action]:
                    ...
                    for action_idx in np.where(agent.stopsignal_applied==1)[0]:
                        ...
                        #print("end stopsignal_applied",agent.stopsignal_applied)
                        #print("end agent.ramping_stopstim_target",agent.ramping_stopstim_target)
                        #print("end agent.stoptimer",agent.stoptimer)
                        #print("end agent.phase",agent.phase)
        ...
        #Stop signal - FSI
                ...
                #for action_idx in range(1):
                    ##action = untrace(actionchannels.iloc[action_idx,0])
                    #trial_wise_stop_onset_2 = self.stop_onset_df_2.iloc[self.trial_num][action_idx]
                    #trial_wise_stop_amplitude_2 = self.stop_amplitude_df_2.iloc[self.trial_num][action_idx]
                    #trial_wise_stop_channel_2 = self.stop_channels_df_2.iloc[self.trial_num][0]

                    #if agent.stoptimer_2 < trial_wise_stop_onset_2 or agent.stopsignal_applied_2[action_idx] == 1 :
                        #continue

                    #if self.stop_signal_present_2 == True:
                        #agent.ramping_stopstim_target_2[action_idx] += trial_wise_stop_amplitude_2
                        #agent.stopsignal_applied_2[action_idx] = 1
                    ##print("0 stopsignal_applied_2",agent.stopsignal_applied_2)
                    ##print("0 agent.ramping_stopstim_target_2",agent.ramping_stopstim_target_2)
                    ##print("0 agent.stoptimer",agent.stoptimer)
                    ##print("0 trial_wise_stop_amplitude_2",trial_wise_stop_amplitude_2)
                    ##print("0 trial_wise_stop_onset",trial_wise_stop_onset)
                    ##print("0 agent.phase",agent.phase)

                #if agent.phasetimer>trial_wise_stop_onset_2+self.stop_duration_df_2.iloc[self.trial_num][action_idx]:
                    ...
                    ##print("2 stopsignal_applied_2",agent.stopsignal_applied_2)
                    ##print("end agent.ramping_stopstim_target_2",agent.ramping_stopstim_target_2)
                        ##print("end agent.stoptimer",agent.stoptimer)
                        ##print("end agent.phase",agent.phase)
